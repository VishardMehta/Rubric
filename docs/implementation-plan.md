...
- [ ] **0.1** Supabase project created and un-paused. Two private storage
      buckets: `introductions`, `answers`
...
- [ ] **0.3** Backend scaffold: `pyproject.toml`, venv, `app/main.py` with CORS
      for `localhost:5173`, `app/config.py` reading env, `app/api/health.py`
- [ ] **0.4** `.env.example` with the four variable names and empty values.
      `.env` gitignored and filled
- [ ] **0.5** `app/heuristics.py` created with every threshold from the specs:
...
- [ ] **1.1** `app/stt.py`, Groq path, `whisper-large-v3-turbo`. Correct
...
- [ ] **2.1** `app/llm.py`: Gemini client, one structured-call helper taking a
      Pydantic model, retry-once-with-violation logic, timeout from
      `heuristics.py`
- [ ] **2.2** `app/models.py`: `Criterion`, `Rubric` per `backend.md` §5.1
- [ ] **2.3** `app/prompts.py`: `rubric_prompts()` returning `(system, user)`
- [ ] **2.4** `app/validation.py`: points sum to exactly 100, 4 to 7 criteria,
      unique slug ids. No silent repair
- [ ] **2.5** `app/storage.py`: Supabase client on `service_role`, job insert
...
## Phase 3 · Screening · 2 days

- [ ] **3.1** `SubScore`, `Screening` models per `backend.md` §5.2, with
      `evidence` as a required quoted span
- [ ] **3.2** `screening_prompts()` builder
- [ ] **3.3** Validation: every criterion once, points in range, sub-scores sum
      to total, evidence appears in the transcript after whitespace
      normalisation
- [ ] **3.4** `app/scoring.py`: band computation from `heuristics.py`
      boundaries. Band is computed server-side and returned, never derived in
      the frontend
- [ ] **3.5** `POST /api/apply/{job_id}` multipart. Upload, transcribe, screen,
      insert. Object path stored, never a URL
...
- [ ] **3.9** **Consistency harness.** One fixture transcript, five runs, assert
      range under the `heuristics.py` threshold
...
- [ ] **4.2** `app/interview.py`: plan generation. Slots 1 to 3 fixed intents,
...
- [ ] **4.5** Question similarity check against all prior questions, threshold
      from `heuristics.py`. Regenerate once naming the prior questions
...
- [ ] **4.13** `app/errors.py`: every code in `backend.md` §9, mapped from
...
- [ ] **5.1** Token layer verified against `DESIGN.md`. Every value present,
      nothing invented
- [ ] **5.2** Primitives: `Button` all four levels with every state, `TextField`,
      `TextArea`, `Select`, `TagInput`, `Chip`, `Spinner`, `Divider`
- [ ] **5.3** `HRShell` with sidebar, `CandidateShell` centered. One nav item,
      no placeholders
- [ ] **5.4** `PageHeader`, `Section`, `Card`, `Split`
- [ ] **5.5** `EmptyState`, `ErrorState`, `LoadingState`, `Toast`, `Modal`
- [ ] **5.6** `api.ts`: typed client, one error shape, all routes
- [ ] **5.7** Focus states visible on every interactive element. Tab through the
...
- [ ] **6.1** `AudioLevelMeter` driven by real `AnalyserNode` input. No
      simulated waveform
- [ ] **6.2** `VoiceRecorder`: `MediaRecorder`, permission handling, Chrome webm
      and Safari mp4, all five states from `screens.md` §6
- [ ] **6.3** Silence detection, 3 seconds, caption in `caution`
- [ ] **6.4** Interview stage 1, ready screen
- [ ] **6.5** Stage 2, question. `display` type, progress track not a stepper,
      auto-record on render or TTS end
- [ ] **6.6** `speechSynthesis` playback with question text always visible
- [ ] **6.7** Stage 3, processing. Labels change on real backend stage
      transitions only. `aria-live="polite"`. Previous question stays dimmed so
      the screen never goes blank
- [ ] **6.8** Stage 4, transition. 320ms, 8px rise, instant under reduced motion
- [ ] **6.9** Error states: transcription failure re-records in the same slot,
      connection loss auto-retries, invalid token full-region
- [ ] **6.10** Refresh mid-interview resumes at the correct question
- [ ] **6.11** Verified at 375px
...
- [ ] **8.7** Candidate Detail split layout, `AudioPlayer`, collapsed
      transcript, approve and reject with confirmation modal, post-approval
      link state
