...
Client project. Status as of 2026-08-17.
...
2. **Question count.** Now fixed at 10 for every interview, at the client's
   request. That runs roughly 13 to 17 minutes, which is long to sit through
   live: for a demo, answer the first few and skip to a pre-completed
   candidate for the result screen. Changing it is one constant,
   `PLAN_TOTAL_QUESTIONS` in `app/core/heuristics.py`.
...
The MVP shipped, then HR accounts and job ownership were added on request
(recorded above as a real scope change), then a 20-part defect report from
the client was worked through. That pass is documented item by item in
`docs/implementation-plan.md`, Phase 11.

Built after the fee was agreed, and worth naming for the same reason the
accounts work is named above:

- HR accounts, sessions and job ownership.
- Screening split into two weighted components, resume 60 / voice 40, each
  a full scoring of the rubric in its own right.
- Interview length fixed at 10 questions with a planned question mix and
  three to four live follow-ups.
- A terminal `hired` state, so the pipeline can end well and not only badly.
- Structured resume and job-description parsing.
- The defect pass: data isolation, Supabase connectivity, screening retry
  budget, stored interview plans, JD replacement, dashboard scores and
  empty states, signed-URL reuse, and stale-state resets.

### Verification at handoff

219 backend tests, ruff, TypeScript, ESLint at zero warnings, and the Vite
build all pass. Against live data: HR isolation partitions 16 of 16
candidates with no overlap, all 29 API routes carry the right
authorization, and interview records show no cross-contamination.

### Setup

`README.md` is the setup document and is the one that ships. Dependencies
are declared in `backend/pyproject.toml` and `frontend/package.json` with
`package-lock.json`; there is deliberately no `requirements.txt`.

### Not shipped

`.gitignore` excludes this file, `CLAUDE.md`, `DESIGN.md`,
`docs/implementation-plan.md`, `docs/prior-art.md`, `docs/reference/`,
`Demofiles/` and `database/seed.sql`. Two reasons, and both matter:
`Demofiles/` holds three real people's resumes, and this file carries
commercial terms. Confirmed untracked with `git ls-files`.
